...
writeClauses(MaxNumTowers):-
    torresenvilage,
    maximunatorre,
    totesciutatsvigildes,
    ciutatsimportants,
    nomestorres(MaxNumTowers),
    relacio,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

torresenvilage:- row(I), col(J), not(posVillage(_,I,J)), writeOneClause([-towerPos(I,J)]), fail.
torresenvilage.

maximunatorre:- village(V), findall(towerPos(I,J), posVillage(V,I,J), L), atMost(1,L), fail.
maximunatorre.

totesciutatsvigildes:- village(V), findall(towerPos(I,J), posWatchesVillage(V,I,J), L), atLeast(1,L), fail.
totesciutatsvigildes.

ciutatsimportants:- significantVillage(V), writeOneClause([hasTower(V)]), fail.
ciutatsimportants.

nomestorres(MaxNumTowers):-findall(hasTower(V), village(V) ,L), atMost(MaxNumTowers,L) ,fail.
nomestorres(_).

relacio:- village(V), findall(towerPos(I,J), posVillage(V,I,J),L), expressOr(hasTower(V), L), fail.
relacio.
...
costOfThisSolution(M,Cost):- findall(V,member(hasTower(V),M),L), sort(L,L1), length(L1,Cost), !.
